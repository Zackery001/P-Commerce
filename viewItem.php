...
if(isset($_POST['search'])){
    $searchedStock = $stock->searchItem($_POST['search']); 
}
...
<div class="container">
	<div class="row">
        ...
        <h2>This the view items page where existing items can be checked</h2>
        <form action="" method="POST">
            <input name = "search" class = "border border-primary mt-2 form-control" rows="1" placeholder="Search.."></input>
        </form>
        <h3>
            <a class="btn btn-sm btn-primary mt-2" href = "viewItem.php" >Refresh</a>
        </h3>
    </div>
</div>
<?php if(!isset($_POST['search'])): ?>
    ...
    <?php foreach($allStock as $stock):?>
        ...
        <?php $updateLink = "updateItem.php?item_id=".$stock[0];?>
        <?php 
            ...
            <div class = "container">
                <div class="row">
                    <div class="mt-2 col-10 offset-1 card card-header bg-transparent border-secondary">
                        ...
                        <div>
                            <h5><?= $stock['details']?></h5>
                            <small> Cost Price : <?= $stock['cost_price']?></small><br>
                            <small> Selling Price : <?= $stock['selling_price']?></small>
        ...
        <div class = "container">
            <div class="row">
                <div class="mt-2 col-10 offset-1 card card-header bg-transparent border-secondary">
                    ...
                    <div>
                        <h5><a href="<?= $viewLink?>"><?= $stock['category']?> :<?= $stock['details']?></a></h5>
                        <small> Cost Price : <?= $stock['cost_price']?></small><br>
                        <small> Selling Price : <?= $stock['selling_price']?></small>
                        <small> Selling Price : <?= $stock['selling_price']?></small>
                        <div class = "col-12" style = "text-align: right">
